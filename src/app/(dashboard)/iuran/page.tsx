...
interface WargaItem {
  ...
  is_kepala?: boolean;
  kelas_iuran?: string; // 'A' | 'B' | 'C'
}

interface KartuKeluargaItem {
  no_kk: string;
  nama_kepala_keluarga?: string;
  kelas_iuran?: string; // Fallback jika kelas diset per KK ('A' | 'B' | 'C')
  data_warga?: WargaItem[];
  warga?: WargaItem[]; // fallback jika relasi di supabase bernama 'warga'
...
export default function IuranPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Fetch Kartu Keluarga dengan Fallback Query (Mencakup data_warga & warga)
  ...
  const getKelasWarga = (kk: KartuKeluargaItem): { kepala: WargaItem | undefined; kelas: string } => {
    ...
    // Deteksi kelas iuran (Prioritas: Kepala Keluarga -> KK level -> Warga pertama -> Default 'B')
    const kelas = (
      kepala?.kelas_iuran || 
      kk.kelas_iuran || 
              ...
              >
                ...
                <div className="space-y-2">
                  {kk.itemsIuran.map((iuran, index) => (
                    ...
                    >
                      {/* Sisi Kiri: Nama & Details Tarif Efektif */}
