...
export default function EditSuratPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const suratId = resolvedParams.id;
  ...
  async function initData() {
    try {
      ...
      if (dataWarga) setListWarga(dataWarga);

      // Fetch data surat yang akan diedit
      ...
      if (surat) {
        ...
        setNikWarga(surat.nik_warga || '');
        setJenisSurat(surat.jenis_surat || 'IJIN_KERAMAIAN');
        setKeterangan(surat.keterangan || '');

        const detail = surat.detail_tambahan || {};
        setNamaPemohon(detail.nama_pemohon || '');
        setCatatan(detail.catatan || '');
        setRt(detail.rt || '001');
        setRw(detail.rw || '010');

        if (detail.nama_pemohon && surat.nik_warga) {
          setSearchWarga(`${detail.nama_pemohon} (${surat.nik_warga})`);
        }
      }
    } catch (err: any) {
      setErrorMsg(err.message || 'Gagal memuat data surat.');
    } finally {
      setLoading(false);
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Gagal memperbarui data surat.');
    } finally {
      setSubmitting(false);
    }
  };
